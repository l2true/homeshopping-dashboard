...
<script>
  ...
  function renderMonth(){
    ...
    weeks.forEach(week => {
      ...
      // 채널마다 고정된 줄을 배정. 같은 채널에 여러 행사가 겹치면 기본은 가장 최근(늦게 시작한) 것 1개만
      // 보여주고, "⋯ N개 더보기"로 나머지를 펼칠 수 있게 함 → 기본 화면은 항상 채널당 1줄.
      const weekKey = toStr(wStart);
      const chLaneBase = {}, chLaneCount = {}, chToggle = {};
      channels.forEach((c,i) => { chLaneBase[c.key]=i; chLaneCount[c.key]=1; });
      let visibleEvs = [];
      channels.forEach(ch => {
        ...
        const expandKey = ch.key + '|' + weekKey;
        const expanded = _monthExpanded.has(expandKey);
        if(laneN <= 1 || expanded){
          chLaneCount[ch.key] = expanded ? laneN + 1 : laneN;  // 펼쳤으면 "접기" 줄 하나 추가
          chEvs.forEach(ev => visibleEvs.push(ev));
          if(expanded) chToggle[ch.key] = {row: laneN, label: '접기 ▲', key: expandKey};
        } else {
          // 최근(늦게 시작한) 것 하나만 lane 0에 표시, 나머지는 숨기고 더보기 토글만 노출
          const mostRecent = chEvs.reduce((a,b) => a._start>b._start ? a : b);
          mostRecent._sublane = 0;
          visibleEvs.push(mostRecent);
          chLaneCount[ch.key] = 2;
          chToggle[ch.key] = {row: 1, label: `⋯ ${chEvs.length-1}개 더보기`, key: expandKey};
        }
      });
      // 채널별 시작 오프셋 누적 (앞 채널이 레인을 더 썼으면 다음 채널은 그만큼 아래로)
      let acc = 0;
      channels.forEach(c => { chLaneBase[c.key]=acc; acc += chLaneCount[c.key]; });
      visibleEvs.forEach(ev => { ev._mlane = chLaneBase[ev.ch] + ev._sublane; });
      const laneCount = Math.max(channels.length, acc);
      ...
      }).join('') + channels.map(ch => {
        const t = chToggle[ch.key];
        if(!t) return '';
        const row = chLaneBase[ch.key] + t.row;
        return `<div class="month-toggle" style="grid-column:1 / span 7;grid-row:${row+2}" onclick="toggleMonthExpand('${t.key}')">${t.label}</div>`;
      }).join('');

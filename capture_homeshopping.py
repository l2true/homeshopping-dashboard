...
def generate_summary(archive_date_dir, hyundai_tab, gs_tab, cj_tab, lotte_tab):
    ...
    for brand, filename, label, extra in [
        ...
        prompt = (
            ...
            + (extra + '\n\n' if extra else '') +
            '규칙:\n'
            '- **최우선 원칙**: 이미지/텍스트에 실제로 보이는 문구만 그대로 사용하세요. '
            '서로 다른 위치의 단어(다른 탭명, 다른 문구 등)를 이어붙이거나 조합해서 새로운 표현을 만들지 마세요. '
            '없는 단어를 추측해서 채워넣지 마세요 (예: "최저가 찬스"를 "최저가 첫구매"로 바꾸는 등 절대 금지). '
            '정확히 어떤 문구인지 확인이 안 되면 그 항목은 생략하세요. 틀린 정보를 만들어내는 것보다 누락이 낫습니다.\n'
            '- 이미지에서 물결표(~)가 붙은 숫자(예: "~10%", "~20%혜택")는 "최대"라는 뜻입니다. '
            '반드시 "최대 10%"처럼 "최대"를 붙여 쓰세요. 물결표를 빼고 그냥 "10%"라고만 쓰면 균일 적용처럼 오해되므로 절대 금지입니다.\n'
            '- **, ##, --- 등 마크다운 기호 절대 사용 금지\n'
            '- 부연 설명, 안내 문구 추가 금지\n'
            ...
            '단, "다드림특가"처럼 상품 카드 위에 반복적으로 붙는 고유 이름이 있는 서브 섹션은 그 섹션 자체가 행사 구성요소이므로 '
            '"특가: (섹션 고유명) (대표 상품·브랜드 1~2개)" 형태로 기록하세요 (예: "특가: 다드림특가 (자뎅, 금빛방앗간 등)"). '
            '모든 상품을 나열하지 말고 눈에 띄는 1~2개만 예시로 드세요.\n'
            '- 반대로 섹션 제목 자체가 특정 브랜드명/상품명인 경우(예: "닥터린 x 뉴트리코어", "해피콜 선물")는 '
            '그 브랜드 하나만의 자체 프로모션이지 행사 전체 혜택이 아니므로, 거기 적힌 사은품·할인(예: "최대 44% 할인+사은품")은 '
            '절대 포함하지 마세요. 사은품·할인은 특히 브랜드 자체 혜택이 섞여 들어가기 쉬우니 "이 혜택이 특정 브랜드 하나에만 '
            '해당하는가?"를 반드시 확인하고, 그렇다면 제외하세요.\n'
            '- 혜택종류가 "특가"인 항목은 (섹션명이 있든 없든) 어떤 상품·브랜드에 적용되는 특가인지 감이 오도록 '
            '대표 상품명이나 브랜드명 1~2개를 괄호로 덧붙이세요 (예: "특가: 최대 90% 할인 (게스·베베드피노 등)", "특가: 오.더.쎄 (침구·가전 등)"). '
            '페이지에 상품명이 전혀 안 보이면 생략해도 됩니다.\n'
            '- 룰렛/뽑기 등 추첨·참여형 이벤트는 "경품"으로 분류하세요.\n'
            '- "OO 신청하기"처럼 버튼/링크만 있고 정확한 금액·조건이 페이지에 나와 있지 않은 항목(예: "선물 지원금 신청하기")은 '
            '실제로 뭘 주는지 확인할 수 없으므로 포함하지 마세요. 신청해야 나오는 정보는 추측하지 말고 생략하세요.\n'
            ...
            '같은 혜택 내용을 서로 다른 혜택종류로 중복해서 적지 마세요 (예: "출석체크 매일 100% 적립금"을 적립과 경품 둘 다에 넣는 것 금지). '
            '적립금을 주는 이벤트는 경품이 아니라 적립으로만 분류하세요. 각 혜택 내용은 가장 적절한 혜택종류 하나에만 한 번 적으세요.\n'
            ...
            'L.CLUB, VIP 등 유료멤버십 관련 배너(예: "L.CLUB 7 Days 적립금 받고 커피도 받아가세요")는 '
            '행사 자체와 무관한 별도의 회원가입/멤버십 홍보이므로 혜택에 포함하지 마세요. '
            '단, 멤버십 등급에 따라 실제 혜택 수치가 다르게 명시된 경우(예: "일반 최대 10%, L.CLUB 최대 20%")는 '
            '그 차이가 진짜 정보이므로 등급명을 그대로 포함해 작성하세요 — 무조건 등급명을 지우지 마세요.\n'
            '혜택이 여러 개면 줄을 나눠 작성하되, 같은 혜택종류는 반드시 하나로 묶어 작성하세요(줄을 나누지 마세요). '
            '상품 카테고리별로 혜택 수치가 다른 경우(예: TV 패션/리빙 12%, TV 가전/가구 2만, 일반상품 10%)에도 '
            '별도 줄로 나누지 말고 한 줄 안에 콤마로 나열하세요 '
            '(예: "적립: TV 패션/리빙 최대 12%, TV 가전/가구 최대 2만, 일반상품 최대 10%").\n'
...
EVENT_ENGINE_JS = r'''
  ...
  function nameKey(n){ return (n||'').toLowerCase().replace(/hot/g,'핫').replace(/\s+/g,'').replace(/[!~·,\/\-_.()]/g,''); }
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .month-toggle { font-size: 10px; font-weight: 700; color: #888; line-height: 17px; height: 17px; padding: 0 7px; cursor: pointer; z-index: 1; }
    .month-toggle:hover { color: #1a1a2e; text-decoration: underline; }
...
<script>
  ...
  let _view = 'week';
  const _monthExpanded = new Set();  // "채널키|주시작일" — 월간 뷰에서 펼쳐진 채널×주
  function toggleMonthExpand(key){
    _monthExpanded.has(key) ? _monthExpanded.delete(key) : _monthExpanded.add(key);
    renderMonth();
  }
  ...
  function renderMonth(){
    ...
    weeks.forEach(week => {
      ...
      // 채널마다 고정된 줄(항상 GS/CJ/롯데/현대 순서)을 기본으로 주되, 같은 채널 안에서
      // 겹치지 않는 행사는 한 줄을 같이 쓰고, 정말 겹칠 때만 그 채널 몫으로 레인을 더 씀.
      // 기본 화면(펼치지 않은 상태)은 채널당 첫 레인만 보여줘 항상 4줄로 고정하고,
      // 넘치는 부분은 날짜별로 "+N개 더보기"를 표시한다(구글 캘린더 방식).
      const weekKey = toStr(wStart);
      const chLaneCount = {};
      channels.forEach(ch => {
        const chEvs = evs.filter(ev => ev.ch===ch.key).sort((a,b)=>a._start-b._start);
        const lanes = [];
        chEvs.forEach(ev => {
          let placed = false;
          for(let li=0; li<lanes.length; li++){
            if(lanes[li] < ev._start){ ev._sublane=li; lanes[li]=ev._end; placed=true; break; }
          }
          if(!placed){ ev._sublane=lanes.length; lanes.push(ev._end); }
        });
        chLaneCount[ch.key] = Math.max(1, lanes.length);
      });
      const hasOverflow = channels.some(c => chLaneCount[c.key] > 1);
      const expanded = _monthExpanded.has(weekKey);

      let visibleEvs, hiddenEvs, visibleLaneCount;
      const chLaneBase = {};
      if(expanded){
        let acc = 0;
        channels.forEach(c => { chLaneBase[c.key]=acc; acc += chLaneCount[c.key]; });
        evs.forEach(ev => { ev._mlane = chLaneBase[ev.ch] + ev._sublane; });
        visibleEvs = evs;
        hiddenEvs = [];
        visibleLaneCount = acc;
      } else {
        channels.forEach((c,i) => { chLaneBase[c.key]=i; });
        visibleEvs = evs.filter(ev => ev._sublane===0);
        visibleEvs.forEach(ev => { ev._mlane = chLaneBase[ev.ch]; });
        hiddenEvs = evs.filter(ev => ev._sublane>0);
        visibleLaneCount = channels.length;
      }
      const laneCount = visibleLaneCount + (hasOverflow ? 1 : 0);
      ...
      const barsHtml = visibleEvs.map(ev => {
        const cs = ev._start<wStart ? wStart : ev._start;
        const ce = ev._end>wEnd ? wEnd : ev._end;
      ...
      if(hasOverflow){
        if(!expanded){
          // 날짜별로 숨겨진 이벤트 개수를 세어 그 요일 칸에만 "+N개 더보기" 표시
          week.forEach(({d},i) => {
            const iso = toStr(d);
            const cnt = hiddenEvs.filter(ev => iso>=toStr(ev._start) && iso<=toStr(ev._end)).length;
            if(cnt>0) moreHtml += `<div class="month-toggle" style="grid-column:${i+1};grid-row:${visibleLaneCount+2}" onclick="toggleMonthExpand('${weekKey}')">+${cnt}개 더보기</div>`;
          });
        } else {
          moreHtml = `<div class="month-toggle" style="grid-column:1 / span 7;grid-row:${visibleLaneCount+2}" onclick="toggleMonthExpand('${weekKey}')">접기 ▲</div>`;
        }
      }

      bodyHtml += `<div class="month-week" style="grid-template-rows:${rowsCss}">${cellsHtml}${barsHtml}${moreHtml}</div>`;
    });

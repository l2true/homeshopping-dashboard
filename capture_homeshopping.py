...
<script>
  ...
  function renderMonth(){
    ...
    weeks.forEach(week => {
      ...
      // 전체 이벤트를 시간순으로 최소 레인에 그리디 배치(주간 뷰와 같은 방식).
      // 채널 구분 없이 한 주에 보여줄 레인 수를 채널 수만큼(기본 4)으로 제한하고,
      // 넘치는 레인에 걸리는 요일에만 그 날짜 기준 "+N개 더보기"를 표시한다(구글 캘린더 방식).
      const weekKey = toStr(wStart);
      const MAX_LANES = channels.length;
      evs.sort((a,b) => a._start-b._start);
      const laneEnds = [];
      evs.forEach(ev => {
        let placed = false;
        for(let li=0; li<laneEnds.length; li++){
          if(laneEnds[li] < ev._start){ ev._mlane=li; laneEnds[li]=ev._end; placed=true; break; }
        }
        if(!placed){ ev._mlane=laneEnds.length; laneEnds.push(ev._end); }
      });
      const totalLanes = Math.max(1, laneEnds.length);
      const expanded = _monthExpanded.has(weekKey);
      const hasOverflow = totalLanes > MAX_LANES;
      const visibleLaneCount = expanded ? totalLanes : Math.min(totalLanes, MAX_LANES);
      const extraRow = hasOverflow;  // "더보기"/"접기" 줄 하나 추가
      const laneCount = visibleLaneCount + (extraRow ? 1 : 0);
      const rowsCss = `18px repeat(${laneCount}, 18px)`;

      const visibleEvs = evs.filter(ev => ev._mlane < visibleLaneCount);
      const hiddenEvs = evs.filter(ev => ev._mlane >= visibleLaneCount);
